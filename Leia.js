// ==UserScript==
// @name         Leia-me Auto Mistral Cheat (Automático)
// @namespace    http://tampermonkey.net/
// @version      5.0
// @description  Responde perguntas e avança automaticamente no Leia-me/Odilo com Mistral AI 😎
// @author       MZ
// @match        *://*odilo*/*
// @grant        none
...
(function () {
    ...
    const API_KEY = 'HJn0dgzp04QzEZkLnMc45lYYQWiIR6QM'; // sua API Key do Mistral
    ...
    style.textContent = `
        ...
        .mistral-box .auto-on { background: #10b981 !important; }
        .mistral-box .auto-off { background: #ef4444 !important; }
    `;
    document.head.appendChild(style);

    const ui = document.createElement("div");
    ui.className = "mistral-box";
    ui.innerHTML = `
        <h1>📘 Leia-me Cheat</h1>
        <h2>🦇 by @mzzvxm</h2>
    ...
    async function chamarMistral(pergunta, alternativas) {
        const prompt = `
Responda a seguinte pergunta do tipo múltipla escolha. Retorne APENAS a letra correta (A, B, C, D ou E). Não inclua nenhum texto adicional.
...
Resposta:
        ...
        try {
            ...
            console.log("💬 Resposta Mistral (raw):", texto);
            console.log("✅ Letra extraída:", letra);

            return letra || null;

        } catch (error) {
            console.error("Erro na chamada Mistral:", error);
            statusDiv.textContent = "Erro na API Mistral";
            return null;
        }
    }
    ...
    async function processarPergunta() {
        ...
        try {
            ...
            statusDiv.textContent = "Pergunta detectada. Chamando Mistral...";
            console.log("📘 Pergunta:", pergunta);
            console.log("🔢 Opções:", opcoes);

            const letra = await chamarMistral(pergunta, opcoes);

            if (letra) {
                statusDiv.textContent = `Resposta Mistral: ${letra}. Selecionando...`;
                console.log("✅ Resposta Mistral:", letra);
                ...
                statusDiv.textContent = "Resposta Mistral não obtida";
